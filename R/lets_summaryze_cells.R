...
#' @param y A \code{data.frame} of per-cell descriptors with the same row order as
...
#' @param func A summary function to aggregate a descriptor across the cells
...
#' @export 
#' 
lets.summaryze.cells <- function(x, y, func = mean) {
  # x[[1]] is expected to be the cell-by-species matrix:
  #  - columns 1:3 = [cell_id, axis1, axis2]
  ...
  # Species columns start at index 4 (as per package convention)
  seq <- 4:n
  
  # Preallocate the output matrix:
  #   rows   = number of species
  #   cols   = number of descriptors in 'y' minus the ID column
  sum_cells <- matrix(
    nrow = n - 3, ncol = ncol(y) - 1,
    dimnames = list(NULL, colnames(y)[-1])
  )
  
  # For each species, aggregate each descriptor across occupied cells
  for (i in seq) {
    # Logical mask of cells where species i is present
    cells <- pam_x[, i] > 0
    
    # Apply the summary function column-wise on descriptors (drop ID col),
    # removing NAs for that species (stats::na.omit)
    sum_cells[i - 3, ] <- apply(
      na.omit(y[cells, -1, drop = FALSE]),
      2, func
    )
  }
  
  # Assemble the final data frame: species names + summarized descriptors
  result <- data.frame(
    "Species" = colnames(pam_x)[seq],
    sum_cells
  )
  
  # Ensure descriptor column names match input 'y'
  colnames(result)[-1] <- colnames(y)[-1]
