...
test_that("lets.attrpam works consistently with different n_bins", {
  for (nb in c(5, 10, 50)) {
    ...
    n_cells_raster <- nrow(res$Attr_Richness_Raster) * ncol(res$Attr_Richness_Raster)
...
test_that("lets.attrpam with remove.sp = TRUE actually removes all-zero species internally", {
  n <- 100
  Species <- rep(paste0("sp", 1:4), each = 25)
  trait_a <- rnorm(n)
  trait_b <- rnorm(n)
  ...
  x_extra <- rbind(x, data.frame(Species = "sp_zero", trait_a = NA, trait_b = NA))
  
  res <- suppressWarnings(lets.attrpam(x_extra, n_bins = 10, remove.sp = TRUE))
