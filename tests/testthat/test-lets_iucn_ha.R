context("Test for lets.iucn.habitats")

sp <- c("Musonycteris harrisoni", "Ailuropoda melanoleuca",
        "Cebus flavius")


test_that("lets.iucn.habitats works fine, one species", {
  skip_on_cran()
...
test_that("lets.iucn.habitats works fine, one species, count = TRUE", {
  skip_on_cran()
...
test_that("lets.iucn.habitats works fine, multiple species", {
  skip_on_cran()
