...
Many times scientists are interested in analyzing or describing the
...
To do this, let’s test the Rapoport’s rule on Phyllomedusa frogs.
...
To start this test we can load our `PresenceAbsence` example object for
the Phyllomedusa species.
...
![](mapping-species-traits_files/figure-html/r-1.png)

Now, we have to calculate the species range sizes. `letsR` includes a
function to do that called `lets.rangesize`. We can calculate the range
size from the `PresenceAbsence` matrix, as the number of cells or in
square meters. Alternatively, we can do it directly on the species
ranges shapefiles. Let’s do the latter option, since it is more
...
To map this trait by cell, we can use the `lets.maplizer` function. You
can choose the function to aggregate the species trait per cell, but in
this case we want to use the default option that will average the
values. We also want the function to return a raster with the mapped
...
![](mapping-species-traits_files/figure-html/unnamed-chunk-3-1.png)

Visually, the Rapoport’s rule doesn’t seem to work on Phyllomedusa
...
![](mapping-species-traits_files/figure-html/unnamed-chunk-5-1.png)

Seems that the Rapoport’s rule doesn’t stand for this genus, actually
...
*A quick note here. Scientists should be careful with this type of
community analysis, as the repetition in species co-occurrence can
easily generate spurious patterns and significant correlations ([see our
paper for further
discussion](https://onlinelibrary.wiley.com/doi/full/10.1111/jbi.12953)).*
